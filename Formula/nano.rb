class Nano < Formula
  ...
  stable do
    ...
    patch :p1, pick_diff(Formula["z80oolong/eaw/nano@4.9.3"].path)
  end
...
index 0d7396f9..1b5216cf 100644
--- a/configure.ac
+++ b/configure.ac
...
index 0725c91d..9aa15be6 100644
--- a/src/chars.c
+++ b/src/chars.c
@@ -28,6 +28,391 @@
 #include <wchar.h>
...
index 91f10d00..703dbc36 100644
--- a/src/definitions.h
+++ b/src/definitions.h
@@ -542,6 +542,12 @@ enum
 	LET_THEM_ZAP,
...
index fe70929d..97be46fc 100644
--- a/src/global.c
+++ b/src/global.c
...
index 8e8b9952..7a124ae2 100644
--- a/src/nano.c
+++ b/src/nano.c
...
@@ -1751,6 +1759,14 @@ int main(int argc, char **argv)
 		{"indicator", 0, NULL, 'q'},
 		{"unix", 0, NULL, 'u'},
 		{"afterends", 0, NULL, 'y'},
+#endif
...
@@ -1785,7 +1801,16 @@ int main(int argc, char **argv)
 #endif
...
@@ -1806,8 +1831,18 @@ int main(int argc, char **argv)
 	if (*(tail(argv[0])) == 'r')
 		SET(RESTRICTED);
...
+#endif /* NO_USE_UTF8CJK */
 ...
 #ifndef NANO_TINY
 			case 'A':
@@ -2041,12 +2076,40 @@ int main(int argc, char **argv)
 			case 'z':
 				SET(SUSPENDABLE);
 				break;
+#ifdef ENABLE_UTF8
...
index 24fb1a36..01919901 100644
--- a/src/prototypes.h
+++ b/src/prototypes.h
...
index 733db378..43d6242a 100644
--- a/src/rcfile.c
+++ b/src/rcfile.c
...
index 89908dbd..95ff4929 100644
--- a/src/winio.c
+++ b/src/winio.c
@@ -29,6 +29,12 @@
 #include <string.h>
...
+#endif /* NO_USE_UTF8CJK */
 ...
 #ifdef REVISION
@@ -1923,7 +1929,23 @@ char *display_string(const char *buf, size_t column, size_t span,
 		}
