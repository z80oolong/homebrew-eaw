class NeomuttAT20230407 < Formula
...
+#endif
     ...
     if (w >= 0)
     {
@@ -832,7 +836,11 @@ void mutt_paddstr(struct MuttWindow *win, int n, const char *s)
     }
     if (!IsWPrint(wc))
       wc = '?';
+#ifdef NO_USE_UTF8CJK
...
+#endif
     ...
     {
       if (w > n)
@@ -877,7 +885,11 @@ size_t mutt_wstr_trunc(const char *src, size_t maxlen, size_t maxwid, size_t *wi
       cl = (cl == (size_t) (-1)) ? 1 : n;
       wc = ReplacementChar;
     }
+#ifdef NO_USE_UTF8CJK
...
@@ -946,7 +958,11 @@ size_t mutt_strnwidth(const char *s, size_t n)
     }
     if (!IsWPrint(wc))
       wc = '?';
+#ifdef NO_USE_UTF8CJK
...
@@ -63,7 +480,12 @@ int mutt_mb_charlen(const char *s, int *width)
   n = mutt_str_len(s);
...
@@ -144,7 +566,12 @@ int mutt_mb_width(const char *str, int col, bool display)
   {
...
@@ -177,7 +604,11 @@ int mutt_mb_width(const char *str, int col, bool display)
  */
...
index d7301a4..30e6627 100644
--- a/mutt_config.c
+++ b/mutt_config.c
@@ -576,6 +576,16 @@ static struct ConfigDef MainVars[] = {
...
index 1b2b4af..9e6fde4 100644
--- a/pager/display.c
+++ b/pager/display.c
@@ -916,7 +916,11 @@ static int format_line(struct MuttWindow *win, struct Line **lines, int line_num
       {
         space = ch;
       }
+#ifdef NO_USE_UTF8CJK
