...
@@ -1804,7 +1820,16 @@ int main(int argc, char **argv)
 #endif
...
@@ -1815,8 +1840,18 @@ int main(int argc, char **argv)
 	if (*(tail(argv[0])) == 'r')
 		SET(RESTRICTED);
...
+				"abcdef:ghijklmno:pqr:s:tuvwxy84$%_!", long_options, NULL)) != -1) {
+#else
+	while ((optchr = getopt_long(argc, argv, "0ABC:DEFGHIJ:KLMNOPQ:RST:UVWX:Y:Z"
+				"abcdef:ghijklmno:pqr:s:tuvwxy8$%_!", long_options, NULL)) != -1) {
+#endif /* NO_USE_UTF8CJK_EMOJI */
+#else
 	while ((optchr = getopt_long(argc, argv, "0ABC:DEFGHIJ:KLMNOPQ:RST:UVWX:Y:Z"
...
+#endif /* NO_USE_UTF8CJK */
 ...
 #ifndef NANO_TINY
 			case 'A':
@@ -2048,6 +2083,21 @@ int main(int argc, char **argv)
 			case 'y':
 				SET(AFTER_ENDS);
 				break;
+#endif
...
+#ifndef NANO_TINY
 			case '%':
 				SET(STATEFLAGS);
 				break;
@@ -2073,6 +2123,21 @@ int main(int argc, char **argv)
 	if (getenv("TERM") == NULL)
 		putenv("TERM=vt220");
