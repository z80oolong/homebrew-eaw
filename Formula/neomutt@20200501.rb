class NeomuttAT20200501 < Formula
  ...
  def install
    ...
    if OS.linux? then
      fix_rpath "#{bin}/neomutt", ["z80oolong/tmux/ncurses-eaw@6.2"], ["ncurses"]
    end
  end
...
index c6bd5e7..10bf6b7 100644
--- a/globals.h
+++ b/globals.h
@@ -287,4 +287,11 @@ WHERE bool C_XCommentTo;                     ///< Config: (nntp) Add 'X-Comment-
...
index 8a0a240..09b2cd2 100644
--- a/gui/curs_lib.c
+++ b/gui/curs_lib.c
@@ -1115,7 +1115,11 @@ void mutt_simple_format(char *buf, size_t buflen, int min_width, int max_width,
 #endif
           if (!IsWPrint(wc))
         wc = '?';
+#ifdef NO_USE_UTF8CJK
...
+#endif
     ...
     if (w >= 0)
     {
@@ -1259,7 +1263,11 @@ void mutt_paddstr(int n, const char *s)
     }
     if (!IsWPrint(wc))
       wc = '?';
+#ifdef NO_USE_UTF8CJK
...
+#endif
     ...
     {
       if (w > n)
@@ -1305,7 +1313,11 @@ size_t mutt_wstr_trunc(const char *src, size_t maxlen, size_t maxwid, size_t *wi
       cl = (cl == (size_t)(-1)) ? 1 : n;
       wc = ReplacementChar;
     }
+#ifdef NO_USE_UTF8CJK
...
@@ -1375,7 +1387,11 @@ int mutt_strnwidth(const char *s, size_t n)
     }
     if (!IsWPrint(wc))
       wc = '?';
+#ifdef NO_USE_UTF8CJK
...
index 59bfb2c..73072ee 100644
--- a/help.c
+++ b/help.c
@@ -163,7 +163,11 @@ static int print_macro(FILE *fp, int maxwidth, const char **macro)
       wc = ReplacementChar;
     }
...
+#endif
     ...
     {
       if (w > n)
@@ -238,7 +242,11 @@ static int get_wrapped_width(const char *t, size_t wid)
     }
     if (!IsWPrint(wc))
       wc = '?';
+#ifdef NO_USE_UTF8CJK
...
index b9bbf23..231aa63 100644
--- a/main.c
+++ b/main.c
@@ -533,6 +533,21 @@ int main(int argc, char *argv[], char *envp[])
...
+{
+  extern bool C_Utf8Cjk;    ///< Config: (utf8cjk) Width of East Asian Ambiguous Charactor is set 2
+#ifndef NO_USE_UTF8CJK_EMOJI
+  extern bool C_Utf8Emoji;  ///< Config: (utf8cjk) Width of UTF8 Emoji is set 2
+#endif
+
+  if (C_Utf8Cjk)
+  {
...
@@ -65,7 +485,12 @@ int mutt_mb_charlen(const char *s, int *width)
...
@@ -146,7 +571,12 @@ int mutt_mb_width(const char *str, int col, bool display)
   {
...
@@ -177,7 +607,11 @@ int mutt_mb_width(const char *str, int col, bool display)
  */
...
index 27c0528..c2379ab 100644
--- a/mutt_config.c
+++ b/mutt_config.c
@@ -4948,6 +4948,12 @@ struct ConfigDef MuttVars[] = {
...
index 2da2b23..14bd241 100644
--- a/pager.c
+++ b/pager.c
@@ -1551,7 +1551,11 @@ static int format_line(struct Line **line_info, int n, unsigned char *buf,
       {
         space = ch;
       }
+#ifdef NO_USE_UTF8CJK
