...
index 18f75e3..a6cd907 100644
--- a/curs_lib.c
+++ b/curs_lib.c
@@ -1315,7 +1315,11 @@ void mutt_format_string (char *dest, size_t destlen,
 #endif
         if (!IsWPrint (wc))
           wc = '?';
...
+#endif
     ...
     if (w >= 0)
     {
@@ -1444,7 +1448,11 @@ void mutt_paddstr (int n, const char *s)
     }
     if (!IsWPrint (wc))
       wc = '?';
+#ifndef NO_USE_MKWCWIDTH
...
+#endif
     ...
     {
       if (w > n)
@@ -1481,7 +1489,11 @@ size_t mutt_wstr_trunc (const char *src, size_t maxlen, size_t maxwid, size_t *w
       cl = (cl == (size_t)(-1)) ? 1 : n;
       wc = replacement_char ();
     }
+#ifndef NO_USE_MKWCWIDTH
...
@@ -1519,7 +1531,11 @@ int mutt_charlen (const char *s, int *width)
...
@@ -1551,7 +1567,11 @@ int mutt_strwidth (const char *s)
     }
     if (!IsWPrint (wc))
       wc = '?';
+#ifndef NO_USE_MKWCWIDTH
...
index 31c2ae4..19d0178 100644
--- a/init.h
+++ b/init.h
@@ -4596,6 +4596,12 @@ struct option_t MuttVars[] = {
...
index 0d2bc9d..290b8a6 100644
--- a/pager.c
+++ b/pager.c
@@ -1267,7 +1267,11 @@ static int format_line (struct line_t **lineInfo, int n, unsigned char *buf,
     {
       if (wc == ' ')
 	space = ch;
+#ifndef NO_USE_MKWCWIDTH
...
index 8fd5e6c..8d1b695 100644
--- a/sendlib.c
+++ b/sendlib.c
@@ -1823,7 +1823,11 @@ static int my_width (const char *str, int col, int flags)
   {
