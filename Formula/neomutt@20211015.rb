class NeomuttAT20211015 < Formula
...
index 50296e4..61e335e 100644
--- a/gui/curs_lib.c
+++ b/gui/curs_lib.c
@@ -767,7 +767,11 @@ void mutt_simple_format(char *buf, size_t buflen, int min_width, int max_width,
 #endif
           if (!IsWPrint(wc))
         wc = '?';
+#ifdef NO_USE_UTF8CJK
...
+#endif
     ...
     if (w >= 0)
     {
@@ -912,7 +916,11 @@ void mutt_paddstr(struct MuttWindow *win, int n, const char *s)
     }
     if (!IsWPrint(wc))
       wc = '?';
+#ifdef NO_USE_UTF8CJK
...
+#endif
     ...
     {
       if (w > n)
@@ -958,7 +966,11 @@ size_t mutt_wstr_trunc(const char *src, size_t maxlen, size_t maxwid, size_t *wi
       cl = (cl == (size_t) (-1)) ? 1 : n;
       wc = ReplacementChar;
     }
+#ifdef NO_USE_UTF8CJK
...
@@ -1028,7 +1040,11 @@ int mutt_strnwidth(const char *s, size_t n)
     }
     if (!IsWPrint(wc))
       wc = '?';
+#ifdef NO_USE_UTF8CJK
...
@@ -64,7 +481,12 @@ int mutt_mb_charlen(const char *s, int *width)
...
@@ -145,7 +567,12 @@ int mutt_mb_width(const char *str, int col, bool display)
   {
...
@@ -176,7 +603,11 @@ int mutt_mb_width(const char *str, int col, bool display)
  */
...
index 9c8a910..b6c59cf 100644
--- a/mutt_config.c
+++ b/mutt_config.c
@@ -657,6 +657,16 @@ static struct ConfigDef MainVars[] = {
...
index 5963c39..0ad1a38 100644
--- a/pager/display.c
+++ b/pager/display.c
@@ -1330,7 +1330,11 @@ static int format_line(struct MuttWindow *win, struct Line **lines, int line_num
       {
         space = ch;
       }
+#ifdef NO_USE_UTF8CJK
